...
function EditCustomerComponent(props) {
    ...
    const { handleEditOpenModal, openEdit, categoryList, rowId, customerList, updateCustomerDetails, updateCustomerDetailsData } = props
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        if (!!isSave && Object.keys(props.updateCustomerDetailsData).length > 0) {
            setIsSave(0)
            handleClose()
            handleEditOpenModal(0, 0, props.updateCustomerDetailsData.is_success,updateCustomerDetailsData.message)
        }
    ...
    const handleSave = () => {
        ...
        const payLoad = { customer_name: state.customer_name, plant_location: state.plant_location, category: state.category, customer_type: state.customer_type, business_type: state.business_type, }
...
export default connect(mapStatetoProps, mapDispatchtoProps)(EditCustomerComponent)