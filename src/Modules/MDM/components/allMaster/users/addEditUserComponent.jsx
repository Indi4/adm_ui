...
import { Autocomplete, FormControl, Box, Button, Grid, IconButton, InputLabel, Modal, TextField, Typography, Tooltip } from "@mui/material";
import CloseIcon from "@mui/icons-material/Close";
import { callCommonUpdateAPI, callCommonSaveAPI, callCommonRefreshProps, callCommonGetAPI } from '../../../../../store/action/action'
import { connect } from 'react-redux'
import { SAVE_USER, GETALL_LIST, UPDATE_USER } from "../../../../endPointConfig";
import { initialState, initialRegisterUser } from "../../../../DataManagement/config";
...
function AddEditUserComponent(props) {
    ...
    const { handleAddEditOpenModal, openPopUp, type, rowId, userList, errorData, getCustomerNameUser, customerNameUserData,
        saveUserDetailsData, saveUserDetails, updateUserDetails, updateUserDetailsData,role } = props
    ...
    useEffect(() => {
        if (customerNameUserData && Object.keys(customerNameUserData).length > 0) {
            if (customerNameUserData.data && Object.keys(customerNameUserData.data).length > 0) {
                setRegisterUser({
                    ...registerUser,
                    roleList: customerNameUserData?.data.roles && customerNameUserData?.data.roles.length > 0 &&
                        customerNameUserData?.data.roles,
                    departmentList: customerNameUserData.data.department_list && customerNameUserData.data.department_list.length > 0 &&
    ...
    useEffect(() => {
        if (!!isSave && Object.keys(saveUserDetailsData).length > 0) {
            setIsSave(0)
            handleClose()
            handleAddEditOpenModal(0, 0, saveUserDetailsData.is_success, "", saveUserDetailsData.message)
        }
        if (!!isSave && Object.keys(updateUserDetailsData).length > 0) {
            setIsSave(0)
            handleClose()
            handleAddEditOpenModal(0, 0, updateUserDetailsData.is_success, "", updateUserDetailsData.message)
        }
    ...
    const valdationForm = () => {
        ...
        if (!registerUser.email || registerUser.email.trim() === "") {
            ...
            valid = false;
        } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(registerUser.email)) {
        ...
        if (!registerUser.department || registerUser.department === "") {
            ...
            const selectedDepartment = registerUser.departmentList?.find(dept => dept.dept_id === registerUser.department);
        ...
        return valid; // Return true if form is valid, else false
    };
    ...
    const handleSave = () => {
        ...
        type === "Add" ? saveUserDetails(`${SAVE_USER}`, payLoad) : updateUserDetails(`${UPDATE_USER}${rowId}`, payLoad);
        setIsSave(1);
    };
    ...
    const handleClose = (e, reason) => {
        ...
        setErrors({}); // Assuming 'setErrors' is the function managing form errors state
                    ...
                    <Grid container spacing={2} style={gridContainerStyle}>
                        ...
                        <Grid item xs={6} marginTop={3}>
                            <FormControl fullWidth>
                                ...
                                <Autocomplete
                                    id="roles"
                                    value={registerUser.role && registerUser.role.charAt(0).toUpperCase() + registerUser.role.slice(1).toLowerCase()}
                                    options={registerUser.roleList && registerUser.roleList?.length > 0 ?
                                        registerUser.roleList?.map((option) => option.charAt(0).toUpperCase() + option.slice(1).toLowerCase())
                                        : []
                                    }
                        ...
                        <Grid item xs={6} marginTop={1}>
                            <FormControl fullWidth>
                                ...
                                <Autocomplete
                                    id="department"
                                    value={registerUser.departmentList && registerUser.departmentList.length > 0 &&
                                        registerUser.departmentList.find(
                                            (el) => el.dept_id === registerUser.department
                                        ) || null
                                    }
                                    options={registerUser?.departmentList && registerUser.departmentList.length > 0
...
const mapDispatchtoProps = (dispatch) => {
    return {
        saveUserDetails: (endPoint, payLoad) => dispatch(callCommonSaveAPI(endPoint, payLoad, 'userDetails')),
        updateUserDetails: (endPoint, payLoad) => dispatch(callCommonUpdateAPI(endPoint, payLoad, 'userDetails')),
        getCustomerNameUser: (endPoint) => dispatch(callCommonGetAPI(endPoint, "customerNameUser")),
        refreshProps: (title) => dispatch(callCommonRefreshProps(title))
    }
}
export default connect(mapStatetoProps, mapDispatchtoProps)(AddEditUserComponent)
