...
const DesignScrap = ({ month, data }) => {
  ...
  if (month) {
    ...
    const monthlyTarget = datasets?.find((dataset) => dataset.label === "Sum of monthly target")?.data || [];
    const monthlyActual = datasets?.find((dataset) => dataset.label === "Sum of monthly Actual")?.data || [];
      ...
      <Card style={{ padding: "20px" }}>
        <ResponsiveContainer width="100%" height={600}>
          <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="target" fill="#FF8080" name="Target" barSize={30} />
